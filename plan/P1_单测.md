...
**进入条件**：P0 完成，`feat/chinese-fts5-bigram` 已推送
**运行环境**：Windows 本地，无需 Hermes 或 Docker
**完成标准**：全部断言通过，无报错
...
单字 CJK 查询（如 `云`、`讯`）在 BEAM 模式的 `len(w) > 1` 过滤下会返回空。这是现有设计边界，不影响日常使用（中文双字词全覆盖）。
...
- [ ] 两组测试均输出 ✅
- [ ] 无 AssertionError 或 ImportError
- [ ] 如有失败，记录具体 case 后回到 P0 修正
