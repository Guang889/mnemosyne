...
def _fts_search(conn: sqlite3.Connection, query: str, k: int = 20) -> List[Dict]:
    ...
    CJK queries use bigram tokenization against the bigram-indexed FTS table.
    Non-CJK queries use stopword-filtered OR terms with synonym expansion.
    """
    if _has_cjk(query):
        import re as _re
        safe_query = _cjk_bigram(_re.sub(r'[^\w\s]', ' ', query))
